...
func TestTextToVideosCreate(t *testing.T) {
	stub := &stubHTTPClient{}
	client := NewClientWithHTTP(stub)
	_, err := client.TextToVideos.Create(context.Background(), TextToVideoParams{
	...
	if stub.method != "POST" || stub.path != "/api/v1/grok_imagine/text_to_videos" {
...
func TestImageToVideosWithTaskID(t *testing.T) {
	stub := &stubHTTPClient{}
	client := NewClientWithHTTP(stub)
	_, err := client.ImageToVideos.Create(context.Background(), ImageToVideoParams{
...
func TestExtensionsCreate(t *testing.T) {
	...
	if stub.path != "/api/v1/grok_imagine/extensions" {
...
func TestUpscalesGet(t *testing.T) {
	...
	if stub.method != "GET" || stub.path != "/api/v1/grok_imagine/upscales/task_upsc_123" {
		t.Fatalf("unexpected request: %s %s", stub.method, stub.path)
	}
}
